 ...
 */
public class TreeOresNormalApples1 extends ItemAppleGold {
    public static final String[] types = new String[]{"iron", "coal", "redstone"};
    @SideOnly(Side.CLIENT)
    private IIcon[] iicon;

    public TreeOresNormalApples1(int healAmount, float saturation, boolean wolf) {
    ...
    protected void onFoodEaten(ItemStack itemStack, World world, EntityPlayer entityPlayer) {
        if (!world.isRemote) {
            ...
            switch (itemStack.getItemDamage()) {
                case 0:
                    //iron
                    entityPlayer.addPotionEffect(new PotionEffect(Potion.resistance.id, 5000, 0));
                    break;
                case 1:
                    //coal
                    entityPlayer.addPotionEffect(new PotionEffect(Potion.nightVision.id, 5000, 0));
                    break;
                case 2:
                    //redstone
                    entityPlayer.addPotionEffect(new PotionEffect(Potion.digSpeed.id, 5000, 0));
                    break;
            }
        } else {
